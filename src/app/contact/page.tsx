...
const About: FC = () => {
  const initialValues: MailWithoutID = {
    name: "",
    email: "",
    subject: "",
    message: "",
  ...
  const handleAddMail = async (values: MailWithoutID, formikHelpers: FormikHelpers<MailWithoutID>) => {
    console.log("Adding mail");
    
    const { status } = await axios.post("https://json-server-trial.vercel.app/mails", values);
    if (status !== 201) {
